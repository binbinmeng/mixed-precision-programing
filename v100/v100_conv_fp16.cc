...
struct TrainingContext {
  ...
  void ForwardPropagation(__half* data,
                          __half* conv1,
                          __half* pconv1,
                          void* workspace) {
    int alpha = 1.0f;
...
int main(int argc,char *argv[ ]) {
  ...
  checkCudaErrors(cudaMalloc(&d_data,
                             sizeof(__half) * context.m_batchSize * channels *
                                 height * width));
  checkCudaErrors(cudaMalloc(&d_conv1,
                             sizeof(__half) * context.m_batchSize *
                                 conv1.out_channels * conv1.out_height *
                                 conv1.out_width));
  checkCudaErrors(cudaMemcpyAsync(d_data,
                                  &img___half[0],
                                  sizeof(__half) * 1 * channels * width * height,
                                  cudaMemcpyHostToDevice));

  // Allocate kernel on GPU
  __half* d_pconv1;
  checkCudaErrors(cudaMalloc(&d_pconv1, sizeof(__half) * conv1.pconv.size()));
  checkCudaErrors(cudaMemcpyAsync(d_pconv1,
                                  &conv1.pconv[0],
                                  sizeof(__half) * conv1.pconv.size(),
                                  cudaMemcpyHostToDevice));
